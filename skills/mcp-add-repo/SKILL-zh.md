...
5. 保护凭据。
   完成标准：不要把上下文里的 secret value、bearer token、API key、cookie、password 或私有凭据直接写入会被 Git 追踪的 repo config。优先使用目标 agent 的环境变量引用机制，例如 `env_vars`、`bearer_token_env_var`、`env_http_headers`、`env` 或 `${NAME}` 占位符。如果用户要求把 credential value 写进 `.codex/config.toml`，先让该文件被 Git ignore 或确认它已被 ignore，然后可以把它作为 local-private config 写入。

6. 设置 Git 追踪策略。
   完成标准：如果仓库使用 Git，用 `git check-ignore -v -- <path>` 检查目标 config 路径。当 `.codex/config.toml` 包含或将包含直接的 credential value 时，写入 credential 前先确保仓库 `.gitignore` 忽略这个精确文件；必要时加入 `.codex/config.toml`。不要加入 `!.codex/config.toml` 或任何会让带凭据 config 被追踪的 negation。只引用环境变量、不含凭据值的 config 可以保持可追踪。
...
9. 验证 config。
   完成标准：编辑后的 config 可以用对应格式的 parser 成功解析，没有重复的 server entry；`.codex/config.toml` 含直接 credential value 时，`git check-ignore -v -- .codex/config.toml` 会报告该文件；diff 只包含预期的 repo-local config 修改、必要的 `.gitignore` 保护，以及用户明确要求的辅助文件。

10. 汇报启用要求。
   完成标准：告诉用户修改了哪个 agent 和哪个 repo-local 文件，是否为 `.codex/config.toml` 添加了 `.gitignore` 保护；说明 trust/approval gate、需要设置的环境变量、login 命令、restart 或 reload 步骤；列出新增或更新的 MCP server。
