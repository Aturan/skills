---
name: backlog
description: 用户明确有创建 Backlog item 的意图（包括为此调用本 skill），或上下文已有目标 Backlog 对象 ID 且 agent 需要读写该对象时，使用 Backlog。
---

# Backlog

把本 skill 作为 Backlog.md 入口。在本 skill 里，`item` 指通过已选择的 Backlog 访问路径管理的 Backlog.md item。
...
搜索、查看、创建、编辑或收尾 item 前，先选择访问路径：

1. 优先使用 Backlog MCP。
   完成标准：如果 `backlog` MCP 工具可用，且能完成请求的 Backlog 操作，则每个 Backlog 操作都使用对应 MCP 能力。

2. 回退到 `backlog-cli.sh`。
   完成标准：如果 Backlog MCP 不可用、缺失，或无法完成请求的 Backlog 操作，则只使用本 skill 目录下的 `./backlog-cli.sh` 访问 Backlog CLI。该脚本默认把 `BACKLOG_CWD` 设为 `~/backlog`，并把参数转发给真实 CLI。

3. 永远不要直接调用 `backlog`。
   完成标准：所有 CLI 读取、搜索、创建、编辑、评论、状态变更、instruction 读取和收尾，都通过 `./backlog-cli.sh` 完成。不要把 `backlog` 作为 shell 命令运行，也不要直接编辑 Backlog task、draft、document、decision 或 milestone markdown 文件。

4. 没有可用访问路径时停止。
   完成标准：如果 Backlog MCP 和 `./backlog-cli.sh` 都不能完成 Backlog 工作，停止 Backlog 操作，并告知用户哪条访问路径失败。
...
在访问路径和安全门禁内，agent 可以按需要管理目标 Backlog item：
...
1. 选择访问路径。
   完成标准：已确认 Backlog MCP 是否可用。MCP 可用时使用 MCP；MCP 不可用时使用 `./backlog-cli.sh`。已排除直接使用 `backlog` CLI。

2. 读取 workflow instructions。
   完成标准：开始 Backlog 工作前，使用已选择的访问路径读取 overview instructions，再根据返回的 instructions 决定下一步。

3. 识别项目。
   完成标准：已经知道 `projectId` 和 `@projectId`；如果有来源证据，则已收集。

4. 验证精确 ID。
   完成标准：用户提供、提到，或当前上下文中存在精确 Backlog item ID（包括 `TNNN` 或 `T-NNN`）时，识别为已有 Backlog 对象，并只使用已选择访问路径的查看能力验证归属。只有 `@projectId` assignee 匹配当前项目时才继续。把这类 ID 识别为目标对象，不代表可以创建新 item。
...
6. 创建 item。
   完成标准：满足安全门禁 2 且没有合适的当前项目 item 时，先通过已选择的访问路径读取 task-creation instructions，然后创建最小、可 review 的 item。item 应包含 `assignee=["@projectId"]`、面向结果的描述、可测试的验收标准、有用的 documentation，以及已知的项目根目录相对 modified files。创建 Git 仓库对应 item 后，使用已选择的访问路径写入包含 `projectId` 和仓库 URL 的来源 comment。

7. 管理已跟踪工作。
   完成标准：实现或修改已跟踪工作前，先通过已选择的访问路径读取 task-execution instructions。按需读取已跟踪状态。在「允许操作」范围内，当任务改变已跟踪工作或 item 必须保持准确时，更新目标 item。只有用户明确有创建 Backlog item 的意图时，才创建拆分 / follow-up item。

8. 收尾 item。
   完成标准：把已跟踪工作标记为完成前，先通过已选择的访问路径读取 task-finalization instructions，并验证 acceptance criteria。只有符合「允许操作」且满足返回 instructions 的完成规则时，才更新已勾选标准、final summary 或完成状态。
