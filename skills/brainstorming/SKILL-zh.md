...
7. **写入缓存报告**：通过讨论检查点后，再按报告模板写入系统缓存目录；如果当前环境无法确定或无法写入系统缓存目录，则写入仓库内的本地缓存目录。完成标准：文件存在，且内容覆盖目标、背景、依据、候选方向、假设和待决问题。
8. **报告自审**：检查报告是否清楚、可追溯、待决透明。完成标准：自审发现的问题已直接修正，或明确留在待决问题中。
